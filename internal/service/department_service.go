...
func (s *DepartmentService) GetWithTree(id uint, depth int, includeEmployees bool, sortBy string) (*domain.Department, error) {
...
func (s *DepartmentService) deleteReassign(tx *gorm.DB, allIDs []uint, reassignTo *uint) error {
	if reassignTo == nil {
		return fmt.Errorf("reassign target required")
	}
