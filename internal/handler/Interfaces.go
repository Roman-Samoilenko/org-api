...
type DepartmentService interface {
	...
	GetWithTree(id uint, depth int, includeEmployees bool, sortBy string) (*domain.Department, error)
	Delete(id uint, mode string, reassignTo *uint) error
}

// EmployeeService — интерфейс сервиса сотрудников.
type EmployeeService interface {
	Create(departmentID uint, fullName, position string, hiredAt *time.Time) (*domain.Employee, error)
}
